...
if (!empty($curp)) {
    ...
    if ($sql_check->get_result()->num_rows > 0) {
        header("Location: /Dashboard Medico Static/Pacientes.php?ok=0&error=" . urlencode("El CURP ya existe"));
        $sql_check->close();
        $conexion->close();
        exit;
    }
...
if ($sql->execute()) {
    ...
    registrar_bitacora(
      ...
      header("Location: /Dashboard Medico Static/Pacientes.php?ok=1");
      $sql->close();
      $conexion->close();
  exit;
} else {
      header("Location: /Dashboard Medico Static/Pacientes.php?ok=0");
      $sql->close();
      $conexion->close();
      exit;
}
