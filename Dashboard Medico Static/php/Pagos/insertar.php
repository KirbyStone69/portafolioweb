...
if (abs($total_calculado - $monto) > 0.01) {
    header("Location: /Dashboard Medico Static/Pagos.php?ok=0&error=" . urlencode("El total no coincide con la suma de servicios"));
    exit;
}
...
try {
    ...
    header("Location: /Dashboard Medico Static/Pagos.php?ok=0&error=" . urlencode($e->getMessage()));
    $conexion->close();
    exit;
}
?>
