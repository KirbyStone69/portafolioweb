...
if ($sql_check->get_result()->num_rows > 0) {
    header("Location: /Dashboard Medico Static/Usuarios.php?ok=0&error=" . urlencode("El usuario ya existe"));
    $sql_check->close();
    $conexion->close();
    exit;
}
...
if ($sql->execute()) {
    ...
    header("Location: /Dashboard Medico Static/Usuarios.php?ok=1");
    $sql->close();
    $conexion->close();
    exit;
} else {
    header("Location: /Dashboard Medico Static/Usuarios.php?ok=0");
    $sql->close();
    $conexion->close();
    exit;
}
