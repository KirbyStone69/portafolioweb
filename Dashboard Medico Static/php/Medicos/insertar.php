...
if (!empty($cedula)) {
    ...
    if ($sql_check->get_result()->num_rows > 0) {
        header("Location: /Dashboard Medico Static/Medicos.php?ok=0&error=" . urlencode("La cédula profesional ya existe"));
        $sql_check->close();
        $conexion->close();
        exit;
    }
...
if ($sql->execute()) {
    ...
    header("Location: /Dashboard Medico Static/Medicos.php?ok=1");
    $sql->close();
    $conexion->close();
    exit;
} else {
    header("Location: /Dashboard Medico Static/Medicos.php?ok=0");
    $sql->close();
    $conexion->close();
    exit;
}
